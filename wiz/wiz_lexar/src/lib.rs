...
use crate::token::RawIdentError;
pub use crate::token::{DocStyle, LiteralKind, Radix, RawStrError, Token, TokenKind};
use std::convert::TryFrom;

/// `wizc` allows files to have a shebang, e.g. "#!/usr/bin/wizrun",
...
/// True if `c` is considered a whitespace according to Wiz language definition.
            ...
            '`' => {
                ...
                if terminated {
                    if is_id_start(first) {
                        TokenKind::RawIdentifier { err: None }
                    } else {
                        TokenKind::RawIdentifier {
                            err: Some(RawIdentError::InvalidStarter { bad_char: first }),
                        }
                    }
                } else {
                    TokenKind::RawIdentifier {
                        err: Some(RawIdentError::NotTerminated),
                    }
                }
            }
            _ => TokenKind::Unknown,
