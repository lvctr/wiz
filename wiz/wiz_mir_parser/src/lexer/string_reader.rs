...
impl<'a> StringReader<'a> {
    ...
    pub fn next_token(&mut self) -> (syntax_token::Spacing, syntax_token::Token) {
        ...
        loop {
            ...
            if text.is_empty() {
                ...
                return (spacing, syntax_token::Token::new(syntax_token::TokenKind::Eof, span))
            }
            ...
            match self.cook_lexer_token(token.kind) {
                ...
                None => {
                    spacing = syntax_token::Spacing::Alone
                }
            }
        }
    }

    fn cook_lexer_token(&mut self, lexer_token_kind: wiz_lexar::TokenKind) -> Option<syntax_token::TokenKind> {
        Some(match lexer_token_kind {
            ...
            wiz_lexar::TokenKind::BlockComment { doc_style, terminated } => {
                if !terminated {
            ...
            wiz_lexar::TokenKind::Identifier => {
                syntax_token::TokenKind::Ident(false)
            }
            ...
            wiz_lexar::TokenKind::UnknownPrefix => {
                syntax_token::TokenKind::Ident(false)
            }
            wiz_lexar::TokenKind::Literal { kind, suffix_start } => {
                syntax_token::TokenKind::Literal(syntax_token::Lit { kind: self.cook_lexer_literal(kind) })
            }
            ...
            wiz_lexar::TokenKind::OpenParen => {syntax_token::TokenKind::OpenDelim(syntax_token::DelimToken::Paren)}
            wiz_lexar::TokenKind::CloseParen => {syntax_token::TokenKind::CloseDelim(syntax_token::DelimToken::Paren)}
            wiz_lexar::TokenKind::OpenBrace => {syntax_token::TokenKind::OpenDelim(syntax_token::DelimToken::Brace)}
            wiz_lexar::TokenKind::CloseBrace => {syntax_token::TokenKind::CloseDelim(syntax_token::DelimToken::Brace)}
            wiz_lexar::TokenKind::OpenBracket => {syntax_token::TokenKind::OpenDelim(syntax_token::DelimToken::Bracket)}
            wiz_lexar::TokenKind::CloseBracket => {syntax_token::TokenKind::CloseDelim(syntax_token::DelimToken::Bracket)}
            ...
            wiz_lexar::TokenKind::Minus => {syntax_token::TokenKind::BinOp(syntax_token::BinOpToken::Minus)}
            wiz_lexar::TokenKind::And => {syntax_token::TokenKind::BinOp(syntax_token::BinOpToken::And)}
            wiz_lexar::TokenKind::Or => {syntax_token::TokenKind::BinOp(syntax_token::BinOpToken::Or)}
            wiz_lexar::TokenKind::Plus => {syntax_token::TokenKind::BinOp(syntax_token::BinOpToken::Plus)}
            wiz_lexar::TokenKind::Star => {syntax_token::TokenKind::BinOp(syntax_token::BinOpToken::Star)}
            wiz_lexar::TokenKind::Slash => {syntax_token::TokenKind::BinOp(syntax_token::BinOpToken::Slash)}
            wiz_lexar::TokenKind::Caret => {syntax_token::TokenKind::BinOp(syntax_token::BinOpToken::Caret)}
            wiz_lexar::TokenKind::Percent => {syntax_token::TokenKind::BinOp(syntax_token::BinOpToken::Percent)}
    ...
    fn cook_doc_comment(&self, comment_kind: syntax_token::CommentKind, doc_style: wiz_lexar::DocStyle) -> syntax_token::TokenKind {
